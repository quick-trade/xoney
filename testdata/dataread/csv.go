...
func LoadChartFromCSV(filePath string) (data.Chart, error) {
	...
	if err != nil {
		return data.Chart{}, fmt.Errorf("Ошибка открытия файла: %w", err)
	}
	defer file.Close()
	...
	if err != nil {
		return data.Chart{}, fmt.Errorf("Ошибка чтения CSV: %w", err)
	}
	...
	for _, row := range rows[1:] {
		...
		if err != nil {
			return data.Chart{}, fmt.Errorf("Ошибка парсинга времени: %w", err)
		}

		open, err := strconv.ParseFloat(openStr, 64)
		if err != nil {
			return data.Chart{}, fmt.Errorf("Ошибка парсинга: %w", err)
		}
		...
		chart.Timestamp = append(chart.Timestamp, timestamp)
